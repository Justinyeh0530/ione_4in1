...
import { CommonService, ActionSyncService } from '../../../services/device/index'
let remote = System._nodeRequire('electron').remote;

@Component({
    selector: 'app-ColorSection',
    templateUrl : './components/support/colorsection/colorsection.component.html',
    styleUrls: ['./components/support/colorsection/colorsection.component.css']
})
...
export class ColorSectionComponent implements OnInit{
    ...
    constructor(
        ...
        this.dotindex = this.actionSyncService.dotindex;
        console.log('ColorSectionComponent loading complete');
        this.subscription = this.actionSyncService.updateColorSection.subscribe((data) => {
            this.ColorSectionArray = data;
            this.initData();
        })
        this.subscription2 = this.actionSyncService.removeColorSection.subscribe((data) => {
        ...
        this.subscription2 = this.actionSyncService.addColorSection.subscribe((data) => {
            let left = 0;
            let color;
            if(this.dotindex != -1) {
            ...
            console.log(actionSyncService.ColorSectionArray)
